...
static size_t parse_hex_bytes(const char *line, unsigned char **out)
{
    ...
    *out = NULL;
    ...
    while (i + 1 < j)
        mem[bi++] = hexpair_to_byte(buf[i], buf[i + 1]), i += 2;
    *out = mem;
    return bytes;
}
...
static void visualize(const RingBuf *rb)
{
    ...
    for (size_t i = 0; i < cap_vis; ++i)
    {
        ...
        if (is_head && is_tail)
            printf(C_HT "HT " C_RESET);
        else if (is_head)
            printf(C_HEAD "H  " C_RESET);
        else if (is_tail)
            printf(C_TAIL "T  " C_RESET);
        else
            printf("   ");
    }
    printf("\n");
...
static void cmd_bench(RingBuf *rb, size_t iters, size_t chunk)
{
    if (chunk == 0)
    {
        printf("bench: chunk 需要 > 0\n");
        return;
    }
    if (chunk > FIXED_CAP)
        chunk = FIXED_CAP; // 我们固定 32 字节上限
...
int main(void)
{
    ...
    for (;;)
    {
        ...
        else if (ieq(cmd, "autoviz"))
        {
            ...
            else if (ieq(args, "off"))
            {
                auto_viz = false;
                printf("autoviz -> off\n");
            }
            else
                printf("用法：autoviz on|off\n");
        }
        else if (ieq(cmd, "cap"))
        {
            printf("capacity = %zu\n", rb_capacity(&rb));
        ...
        else if (ieq(cmd, "pushx"))
        {
            ...
            if (n == 0)
            {
                printf("pushx: 解析失败（示例：01 02 0xFF DEADBEEF）\n");
                continue;
            }
        ...
        else if (ieq(cmd, "searchs"))
        {
            ...
            size_t idx = 0;
            int found = rb_search(&rb, args, strlen(args), &idx);
